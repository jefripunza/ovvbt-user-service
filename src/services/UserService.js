...
exports.insertCustomer = async (traceId, { name, email, password }) => {
  let message = "";
  try {
    ...
    print(traceId, { error: "user > insertByAdmin", message });
    return {
      code: 500,
      message,
    };
...
exports.insertByAdmin = async (
  ...
  try {
    ...
    if (!(name && phone_number && email && password && role)) {
      message = "body (name, phone_number, email, password, role) require!";
      print(traceId, { message });
      return {
        code: 400,
        message,
...
exports.detail = async (traceId, { phone_number }) => {
  let message = "";
  try {
    // check apakah phone_number sudah ada di database
    const user = await UsersRepo.isPhoneNumberExist(phone_number);
    if (user) {
      return {
        code: 200,
        render: {
    ...
    message = `phone_number "${phone_number}" tidak terdaftar`;
    print(traceId, { message });
    return {
      code: 400,
      message,
...
exports.validation = async (traceId, { phone_number }) => {
  let message = "";
  try {
    // check apakah phone_number sudah ada di database
    let user = await UsersRepo.isPhoneNumberExist(phone_number);
    if (user) {
...
exports.validationLogin = async (traceId, { email, password }) => {
  let message = "";
  try {
    // check apakah phone_number sudah ada di database
    if (!(email && password)) {
    ...
    const isLogin = await UsersRepo.isLogin(email, password);
    if (!isLogin) {
...
exports.updateOtp = async (traceId, { phone_number, ref_id, otp }) => {
  let message = "";
  try {
    // phone_number (selector) wajib
    if (!phone_number) {
      message = "phone_number dibutuhkan di body";
      print(traceId, { message });
      return {
        code: 405,
        message,
    ...
    // update data
    const { affected } = await UsersRepo.updateWherePhoneNumber(phone_number, {
      ref_id,
      otp,
    });
...
exports.updateAdmin = async (
  ...
  try {
    ...
    if (!(phone_number || name || dob || gender || religion)) {
      message =
        "wajib mengisi salah satu body (phone_number, name, dob, gender, religion)";
      print(traceId, { message });
      return {
        code: 405,
        message,
    ...
    // check apakah phone_number sudah terdaftar di database
    const isRegistered = await UsersRepo.isPhoneNumberExist(phone_number);
    if (!isRegistered) {
...
exports.update = async (traceId, id_user, { email, phone_number, name }) => {
  let message = "";
  try {
    // check apakah ada yang ingin dirubah
    if (!(email || phone_number || name)) {
      message = "wajib mengisi salah satu body (email, phone_number, name)";
      print(traceId, { message });
      return {
        code: 405,
        message,
